...
import { JSONFile, Low } from 'lowdb'; // eslint-disable-line improt/no-unresolved
...
const saveData = async (I: http.IncomingMessage, 0: http.OutgoingMessage) => {
  const doResponse = (code: number = 0, message: string = 'Save succesful') : object => {
  ...
  receivedData.forEach((obj: { [key: string]: string; }) => {
    ...
    const vTime: FixedLengthArray<number, 2> = [momnet().valueOf(), Number.parseFloat(v)];
